...
\caption{\label{code:1} \emph{Poor Mans Rendering \textbf{with} Label Volume}}
\begin{algorithmic}
\REQUIRE \quad\\
...
What makes Poor Mans Rendering such an attractive trade-off is that the visual impact of the expected irregularities is near insignificant and thus acceptable given the boost in performance (relative full neighborhood analysis).


\section{Poor Mans Rendering Without Label Volume}

In this section we will present a data encoding scheme that eliminates the need to upload the label volume to the GPU. The encoding relies on a set of linear mappings (one for each class) from the source volume to an encoded target volume. The key here is to ensure that the co-domain of the mappings each span a unique value range in the target volume. 

For example, say we have three classes which voxels exhibit overlapping value ranges ($0.3$--$0.5$, $0.2$--$0.7$ and $0.1$--$0.5$). We then map the value rangesof of these classes to three unique value ranges in the target volume
\begin{align}
...
\end{align}
From this, the attenuation parameter $t$ can be computed as 
\begin{align}
t &= \mathrm{abs}(v_\mathrm{lin} - \frac{e_\mathrm{max} + e_\mathrm{min}}{2}) - \frac{e_\mathrm{max} - e_\mathrm{min}}{2}  \label{eq:attenuation2}
\end{align}
clamped to the $0$--$1$ range. Poor Mans Rendering can now be performed through Equation~\ref{eq:attenuation2} and Equation~\ref{eq:alpha}. The full process is described in Algorithm~\ref{code:2}.
...
\begin{algorithm}[t]
%\renewcommand{\thealgorithm}{1}
\caption{\label{code:2} \emph{Poor Mans Rendering \textbf{without} Label Volume}}
\begin{algorithmic}
\REQUIRE \quad\\
