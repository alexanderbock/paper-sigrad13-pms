...
The basic idea behind rendering segmented data is identical to standard volume rendering: to select visual parameters based on the class membership of each sample\cite{kaufman2000real, VisHandbook, udupa2000three}. This is greatly simplified since the required class membership can be accessed directly though the label volume. Therefore, the visual properties are often defined on a per-class basis, rather than in a single global definition for the entire data. The complexity of the per-class visual properties vary depending on what the situation requires and can span from a unique color to a fully defined class-specific transfer function. In this paper we use a single transfer function per class but apply the same shading scheme to all classes, however, it should be noted that this is a stylistic choice rather than a requirement of the approach.
...
\begin{figure}
	...
	\subfigure [Two-Level Volume Rendering] {
		\includegraphics[width=0.45\columnwidth]{figures/walnut_dual_full_singleslice_1.png}
		\label{fig:result:single:twolevel}
	}
	\subfigure [Poor Man's Rendering] {
		\includegraphics[width=0.45\columnwidth]{figures/walnut_dual_pms_singleslice_1.png}
...
\begin{figure*}
	\centering
	\subfigure [Two-Level Volume Rendering] {
		\includegraphics[width=0.75\columnwidth]{figures/walnut_dual_full_slices_1_cut.png}
		\label{fig:result:dual:twolevel}
	}
	\subfigure [Poor Man's Rendering] {
		\includegraphics[width=0.75\columnwidth]{figures/walnut_dual_pms_slices_1_cut.png}
		\label{fig:result:dual:pms}
	}
	\subfigure [Two-Level Volume Rendering without Label Volume] {
		\includegraphics[width=0.75\columnwidth]{figures/walnut_single_full_slices_1_cut.png}
		\label{fig:result:single:twolevel}
	}
	\subfigure [Poor Man's Rendering without Label Volume] {
		\includegraphics[width=0.75\columnwidth]{figures/walnut_single_pms_slices_1_cut.png}
		\label{fig:result:single:pms}
	}
	\subfigure [Nearest Neighbor] {
		\includegraphics[width=0.75\columnwidth]{figures/walnut_dual_nearest_slices_1_cut.png}
		\label{fig:result:nn}
	}
...
\begin{figure*}
	\centering
	\subfigure[Two-Level Volume Rendering] {
		\includegraphics[width=\columnwidth]{figures/walnut_rendering_full.png}
		\label{fig:result2:twolevel}
	}
	\subfigure[Poor Man's Rendering] {
		\includegraphics[width=\columnwidth]{figures/walnut_rendering_pms.png}
		\label{fig:result2:pms}
	}
	\subfigure[Difference Image] {
		\includegraphics[width=\columnwidth]{figures/walnut_rendering_1diff.png}
		\label{fig:result2:diff}
	}
	\subfigure[Difference Image (x10)] {
		\includegraphics[width=\columnwidth]{figures/walnut_rendering_10diff.png}
...
In this section we present the results that we achieve using our two proposed techniques. The real world dataset we use is the Walnut with its accompanying segmentation, which we transformed into the encoded format for testing of the rendering without a label volume. 
