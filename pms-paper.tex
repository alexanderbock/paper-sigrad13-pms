...
\FORALL {points $x$ along ray} \nonumber
\STATE Hello World!
%\STATE 1)~ Sample grid neighbors $\x_i$ in $\Omega$ and compute $P(\e_j\mid\x_i)$
%\FORALL {features $\e_j \in \E$} \nonumber
%\STATE 2)~ Reconstruct $\hat{z}_{\e_j}(x)$ w.r.t. feature $\e_j$ (Equation~\ref{eq:feature-estimation})
%\STATE 3)~ Compute reconstruction validity $\V_{\e_j}$ (Equation~\ref{eq:kernel-validity})
%\FORALL {visual primitives $\mTF_i$ associated with $\e_j$} \nonumber
%\STATE 4)~ Extract visual properties $C_i$, $\alpha_i$ through $\mTF_i\big(\hat{z}_{\e_j}(x)\big)$
%\STATE 5)~ Store visual contribution $\Frag_i = \{C_i,\alpha_i,\V_{\e_j}\}$
%\ENDFOR
%\ENDFOR
%\STATE 6)~ Compute final buffer increment at point $x$ as a combination of stored contributions $\{\Frag_i\}$ (see Section~\ref{sec:visual-contrib})
\ENDFOR
\end{algorithmic}
\end{algorithm}
